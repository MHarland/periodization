...
class LatticeGreensfunction(LatticeGreensfunctionGen, LatticeSelfenergyGen):
    def __init__(self, blocknames, blockindices, r, hopping_r, nk, selfenergy, mu, weights_r, *args, **kwargs):
        ...
        if not hk_on_the_fly:
            ...
            r = np.array(r)
            d = len(r[0])
            k_mesh = np.array([k for k in itt.product(
                *[np.linspace(-.5, .5, nk, False)]*d)])
            hk = HkOnTheFly(hopping_r, r, k_mesh)
            LatticeSelfenergyGen.__init__(
                self, blocknames, blockindices, r, None, nk, hk, k_mesh, *args, **kwargs)
        self.wr = weights_r
        self.gr = {}
        self.ginvr = {}
        if not gk_on_the_fly:
            ...
            self.gk = GkOnTheFly(blocknames, blockindices,
                                 selfenergy, mu, self.hk)
...
class GkOnTheFly:
    def __init__(self, blocknames, blockindices, selfenergy, mu, hk):
        ...
        self.mu = mu
        self.g_ki = BlockGf(name_block_generator=[(bn, GfImFreq(
            mesh=selfenergy.mesh, indices=bi)) for bn, bi in zip(blocknames, blockindices)], make_copies=False)

    def __getitem__(self, i_k):
        for bn, b in self.g_ki:
            b << inverse((iOmega_n + self.mu) -
                         self.hk[i_k][bn] - self.selfenergy[bn])
        return self.g_ki
...
class HkOnTheFly:
    def __init__(self, hopping_r, r, k_mesh):
        ...
        self.k = k_mesh
        self.nk = len(self.k)
        self.hr = [{bn: np.array(b, dtype=complex)
                    for bn, b in hr.items()} for hr in hopping_r]
