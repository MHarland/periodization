...
import numpy as np
import h5py
from pytriqs.gf import BlockGf, GfImFreq, GfImTime
...
class TestLatticeGreensfunction(unittest.TestCase):
    ...
    def test_LatticeGreensfunction_full(self):
        ...
        spins = ['up', 'dn']
        g_loc = BlockGf(name_block_generator=[
                        (s, GfImFreq(beta=gimp.mesh.beta, indices=[0])) for s in spins], make_copies=False)

        g_loc['up'][0, 0] << (gimp['up-+'][0, 0]+gimp['up--']
                              [0, 0]+gimp['dn-+'][0, 0]+gimp['dn--'][0, 0]) * .25
        #g_loc['up'][0,0] << (gimp['up'][0,0]+gimp['up'][1,1]+gimp['dn'][0,0]+gimp['dn'][1,1]) * .25
        #g_loc['up'][0,0] << (gimp['up'][0,0]+gimp['up'][1,1]+gimp['dn'][0,0]+gimp['dn'][1,1]) * .25
        g_loc['dn'] << g_loc['up']
        g_nn = BlockGf(name_block_generator=[
                       (s, GfImFreq(beta=gimp.mesh.beta, indices=[0])) for s in spins], make_copies=False)
        g_nn['up'][0, 0] << (gimp['up-+'][0, 0]-gimp['up--']
                             [0, 0]+gimp['dn-+'][0, 0]-gimp['dn--'][0, 0]) * .25
        ...
        glat = LatticeGreensfunction(
            blocknames, blockinds, translations, hopping_r, nk, g_r, weights_r)
        eps = [val[0, 0].real for h in glat.hk for key,
