...
class TestLatticeSelfenergy(unittest.TestCase):
    ...
    def test_LatticeSelfenergy_full(self):
        ...
        spins = ['up', 'dn']
        sig_loc = BlockGf(name_block_generator=[(s, GfImFreq(
            beta=sigmaimp.mesh.beta, indices=[0])) for s in spins], make_copies=False)

        sig_loc['up'][0, 0] << (sigmaimp['up-+'][0, 0]+sigmaimp['up--']
                                [0, 0]+sigmaimp['dn-+'][0, 0]+sigmaimp['dn--'][0, 0]) * .25
        #sig_loc['up'][0,0] << (sigmaimp['up'][0,0]+sigmaimp['up'][1,1]+sigmaimp['dn'][0,0]+sigmaimp['dn'][1,1]) * .25
        #sig_loc['up'][0,0] << (sigmaimp['up'][0,0]+sigmaimp['up'][1,1]+sigmaimp['dn'][0,0]+sigmaimp['dn'][1,1]) * .25
        sig_loc['dn'] << sig_loc['up']
        sig_nn = BlockGf(name_block_generator=[
                         (s, GfImFreq(beta=sigmaimp.mesh.beta, indices=[0])) for s in spins], make_copies=False)
        sig_nn['up'][0, 0] << (sigmaimp['up-+'][0, 0]-sigmaimp['up--']
                               [0, 0]+sigmaimp['dn-+'][0, 0]-sigmaimp['dn--'][0, 0]) * .25
        ...
        slat = LatticeSelfenergy(
            blocknames, blockinds, translations, hopping_r, nk, sigma_r, weights_r)
