...
from pytriqs.gf.local import BlockGf, GfImFreq, GfImTime, iOmega_n, inverse
...
class TestLatticeCumulant(unittest.TestCase):
    ...
    def test_LatticeCumulant_full(self):
        ...
        m_loc = BlockGf(name_block_generator = [(s, GfImFreq(beta = sigmaimp.beta, indices = [0])) for s in spins])

        m_loc['up'][0,0] << (mimp['up-+'][0,0]+mimp['up--'][0,0]+mimp['dn-+'][0,0]+mimp['dn--'][0,0]) * .25
        m_loc['dn'] << m_loc['up']
        m_nn = BlockGf(name_block_generator = [(s, GfImFreq(beta = sigmaimp.beta, indices = [0])) for s in spins])
        m_nn['up'][0,0] << (mimp['up-+'][0,0]-mimp['up--'][0,0]+mimp['dn-+'][0,0]-mimp['dn--'][0,0]) * .25
        m_nn['dn'] << m_nn['up']
        ...
        slat = LatticeSelfenergy(blocknames, blockinds, translations, hopping_r, nk, m_r, weights_r, mu)
